      ...
      <el-form :model="StaffForm" ref="StaffForm" :rules="rules">
       ...
       <!-- <el-form-item label="确认查台密码" prop="QueryPwd" :label-width="formLabelWidth">
        ...
        <el-form-item label="工号" prop="empno" :label-width="formLabelWidth">
          <el-input v-model="StaffForm.empno" :readonly="true" autocomplete="off"></el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add: function() {
        this.title="员工新增";
        this.StaffForm.empname=null;
        this.StaffForm.empno=null;
        this.StaffForm.pwd=null;
        this.StaffForm.querypwd=null;
        this.StaffForm.roleid=null;
        this.StaffForm.empunit=null;
        this.StaffForm.remark=null;
         this.StaffForm.disabled=null;
        this.dialogFormVisible = true;

      },
      /* //新增修改提交的方法 */
      doSubmit: function() {
        alert(this.StaffForm.disabled)
        this.$refs['StaffForm'].validate((valid) => {
          console.log(valid);
          if (valid) {
            console.log("........");
            let url = 'http://localhost/wuliuxm/insertSyEmpLx';
            if (this.title == '员工编辑') {
            ...
            let pages = {
             ...
             empname: this.StaffForm.empname,
             empno:this.StaffForm.empno,
             pwd:this.StaffForm.pwd,
             querypwd :this.StaffForm.querypwd,
             roleid:this.StaffForm.roleid,
             empunit:this.StaffForm.empunit,
             remark :this.StaffForm.remark,
             disableds:this.StaffForm.disabled,
            //disabled:0,
              //disabled:this.StaffForm.disabled=null;
           //  disabled:0;
           /*  if(aa.equles("是")){
               disabled:1
             } */
      ...
      query: function() {
        ...
        let url = 'http://localhost/wuliuxm/FillAllSyEmpZhuLx';
        axios.post(url, qs.stringify(pages)).then(resps => {
          console.log("abc" + resps)
          this.result = resps.data;
        }).catch(error => {
      ...
      max: function() {
        ...
        //console.log(qs.stringify(pages))
        let url = 'http://localhost/wuliuxm/MaxSyEmpLx';
        axios.post(url, qs.stringify(pages)).then(resps => {
          /* var jsonData = JSON.stringify(pages);// 转成JSON格式
          var result = $.parseJSON(jsonData);// 转成JSON对象 */
          console.log("最大最" + resps)
          this.total = resps.data;
        //  console.log("Max00" + this.total)
        }).catch(error => {
          console.log(error);
        });
      },
